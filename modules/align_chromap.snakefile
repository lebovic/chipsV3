...
rule chromap:
    ...
    params:
        ...
        in_files = lambda wildcards, input: f"-1 {input[0]} -2 {input[1]}" if len(input) > 1 else f"-1 {input[0]}"
    threads: _chromap_threads
    message: "ALIGN: Running Chromap for alignment"
    log: output_path + "/logs/align_chromap_{sample}.log"
    conda: "../envs/align/align_chromap.yaml"
    #run:
    #    # single-end
    #    if len(input) > 1:
    ...
    shell:
        """chromap -x {params.index} -r {params.ref} {params.in_files} -t {threads} --SAM -o {output.sam} --trim-adapters -l 2000 -q 0 2 && samtools view -Sb {output.sam} > {output.bam} 2>>{log}"""
