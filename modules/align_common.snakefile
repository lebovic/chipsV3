...
rule align_mapFigure:
    ...
    conda: "../envs/align/align_common.yaml"
    shell:
        src_path + "/modules/scripts/align_mappedFigure.py -f {input} -o {output}"
...
rule align_collectMapStats:
    ...
    #run:
    #    files = " -f ".join(input)
    #    shell(src_path + "/modules/scripts/align_getMapStats.py -f {files} > {output} 2>>{log}")
    shell:
        src_path + "/modules/scripts/align_getMapStats.py {params.files} > {output}"
...
rule align_readsPerChromStat:
    ...
    conda: "../envs/align/align_common.yaml"
    shell:
        src_path + "/modules/scripts/align_readsPerChrom.sh -a {input.bam} > {output} 2>> {log}"
